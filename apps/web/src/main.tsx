...
function pill(status?: Status | null) {
...
function App() {
  ...
  const [sub, setSub] = useState<'bind' | 'recycle' | null>(null);
      ...
      <div className="device">
        <div className="screen" id="screen">
          ...
          <div className="pages">
            ...
            <section className={`page ${tab === 'history' ? 'on' : ''}`}><History records={records} metric={histMetric} setMetric={setHistMetric} /></section>
            <section className={`page ${tab === 'stats' ? 'on' : ''}`}><Stats records={records} metric={statMetric} setMetric={setStatMetric} me={me} /></section>
            <section className={`page ${tab === 'mine' ? 'on' : ''}`}><Mine me={me} refresh={refresh} showToast={showToast} openBind={() => setSub('bind')} /></section>
          ...
          <div className={`mask ${sheetOpen || modal ? 'on' : ''}`} onClick={() => { setSheetOpen(false); setModal(null); }} />
...
function tabTitle(tab: Tab) {
  return { home: '糖迹', history: '历史记录', stats: '健康统计', mine: '我的' }[tab];
}

function StatusBar() {
  return <div className="statusbar"><span className="num">9:41</span><span className="icons">▰▰▰</span></div>;
}
...
function TabBar({ tab, setTab, openSheet }: { tab: Tab; setTab: (tab: Tab) => void; openSheet: () => void }) {
  const item = (id: Tab, label: string, path: string) => <button className={`tab ${tab === id ? 'on' : ''}`} onClick={() => setTab(id)}><svg viewBox="0 0 24 24"><path d={path} /></svg>{label}</button>;
  return (
    <nav className="tabbar">
      {item('home', '首页', 'M3.5 10.5 12 3.5l8.5 7v9a1.5 1.5 0 0 1-1.5 1.5h-4.5v-6h-5v6H5a1.5 1.5 0 0 1-1.5-1.5z')}
      {item('history', '历史', 'M4 5.5h16M4 12h16M4 18.5h10')}
      <div className="fab-slot"><button className="fab" onClick={openSheet}><svg viewBox="0 0 24 24" fill="none"><path d="M12 2.8S5.6 9.7 5.6 14.4a6.4 6.4 0 0 0 12.8 0C18.4 9.7 12 2.8 12 2.8z" fill="#fff" fillOpacity=".22" stroke="#fff" strokeWidth="1.7" /><path d="M12 9.2v6.4M8.8 12.4h6.4" stroke="#fff" strokeWidth="1.9" strokeLinecap="round" /></svg></button><span className="fab-label">记一笔</span></div>
      {item('stats', '统计', 'M4 19.5h16M7 16v-5M12 16V7.5M17 16v-3')}
      {item('mine', '我的', 'M12 11.8a3.6 3.6 0 1 0 0-7.2 3.6 3.6 0 0 0 0 7.2M4.8 20a7.6 7.6 0 0 1 14.4 0')}
    </nav>
  );
}
...
function Home({ overview, me, setTab, setStatMetric }: any) {
  ...
  return (
    <>
      <div className="greet"><div><h2>上午好</h2><div className="date">6月12日 周五 · 今天已记 {overview?.todayCount ?? 0} 笔</div></div><div className="streak">🔥 连续记录 {overview?.streak ?? 0} 天</div></div>
      <div className="hero">
        {latest ? (
          <>
            <div className="lab"><span>血糖 · 最近一次 · {latest.periodName}</span><span>{fmtMD(latest.measuredAt)} {fmtTime(latest.measuredAt)}</span></div>
            <div className="read"><span className="v num" style={{ color: statusColor[latest.status.key] }}>{displayGlucose(latest.valueMmol, me?.unit || 'mmol')}</span><span className="u">{me?.unit === 'mgdl' ? 'mg/dL' : 'mmol/L'}</span></div>
            <div className="meta">{pill(latest.status)}<span>目标 {target.fastingLow}–{target.postMealHigh} mmol/L</span></div>
            <div className="gauge"><span className="band" style={{ left: `${((target.fastingLow - 2) / 18) * 100}%`, width: `${((target.postMealHigh - target.fastingLow) / 18) * 100}%` }} /><span className="dot" style={{ left: `${dotLeft}%`, background: statusColor[latest.status.key] }} /></div>
            <div className="gx"><span>2.0</span><span>10.0</span><span>20.0</span></div>
          </>
...
function MetricCard({ metric, overview, setTab, setStatMetric, me }: any) {
  ...
  return (
    <div className="mcard" onClick={() => { setStatMetric(metric); setTab('stats'); }}>
      <div className="brow"><span className="mic" style={{ background: meta.soft, color: meta.c }}><MetricIcon metric={metric} /></span><span className="bt">{meta.n} · 最近一次</span><span className="when">{latest ? fmtTime(latest.measuredAt) : ''}</span></div>
      {!latest ? <div className="empty" style={{ padding: '8px 0' }}>还没有{meta.n}记录 · 点下方按钮记一笔</div> : metric === 'bp' ? (
        <><div className="mainrow"><span className="mv num" style={{ color: statusColor[latest.status.key] }}>{latest.sbp}/{latest.dbp}</span><span className="mu">mmHg · ♥ {latest.pulse || '-'}</span>{pill(latest.status)}</div><div className="sub">家庭自测参考 &lt;135/85</div></>
      ) : metric === 'lipid' ? (
        <><div className="lgrid">{lipidItems.map(([k, , ab]) => <div className="li" key={k}><span>{ab}</span><b className="num" style={{ color: latest.itemStatus[k] ? statusColor[latest.itemStatus[k]] : 'var(--ink-3)' }}>{latest[k] ?? '—'}</b></div>)}</div><div className="sub">{pill(latest.status)}</div></>
      ) : (
        <><div className="mainrow"><span className="mv num" style={{ color: statusColor[latest.status.key] }}>{latest.value}</span><span className="mu">μmol/L</span>{pill(latest.status)}</div><div className="sub">参考上限 &lt;{uricThreshold(me?.sex)}</div></>
      )}
    </div>
  );
}

function History({ records, metric, setMetric }: any) {
  return (
    <>
      ...
      {(records[metric] || []).length === 0 ? <div className="empty">暂无{metrics.find((m) => m.k === metric)?.n}记录</div> : records[metric].map((r: any) => <RecordRow key={r.id} metric={metric} record={r} />)}
      <div className="foot-note">点击记录可编辑或删除 · 删除后 7 天内可在回收站找回</div>
    </>
  );
}

function RecordRow({ metric, record }: { metric: Metric; record: any }) {
  const status = record.status;
  const read = metric === 'glucose' ? `${record.displayValue} mmol/L` : metric === 'bp' ? `${record.sbp}/${record.dbp} mmHg` : metric === 'uric' ? `${record.value} μmol/L` : lipidItems.filter(([k]) => record[k] != null).map(([k, , ab]) => `${ab} ${record[k]}`).join(' · ');
  return (
    <div className="rec">
...
function Stats({ records, metric, setMetric, me }: any) {
...
function Mine({ me, refresh, showToast, openBind }: any) {
  ...
  return (
    <>
      ...
      <div className="card">{me?.binding ? <div className="bound-cell"><div className="bi"><div className="bn">{me.binding.pharmacyName}</div><div className="bs">{fmtMD(me.binding.boundAt)} 起 · 该药房可查看你的健康记录</div></div><button className="danger-text" onClick={unbind}>解绑</button></div> : <div className="cell" onClick={openBind}><div className="cm"><div className="ct" style={{ color: 'var(--brand)' }}>＋ 绑定服务药房</div><div className="cs">输入药房邀请码，获得用药与健康管理服务</div></div></div>}</div>
...
function RecordSheet({ open, metric, setMetric, me, refresh, close, showToast, setModal }: any) {
  ...
  useEffect(() => { if (open) { const d = new Date(); setTime(toLocalInput(d)); setPeriod(metric === 'bp' ? inferBpPeriod(d) : inferGlucosePeriod(d)); setBuf(''); setTags([]); setBp({ f: 'sbp', sbp: '', dbp: '', pulse: '' }); } }, [open, metric]);
  ...
  async function save() {
    try {
      ...
      if (metric === 'glucose') {
        ...
        body = { value: Number(buf), unit: me?.unit || 'mmol', period, measuredAt, tags };
      } else if (metric === 'bp') {
        if (!bp.sbp || !bp.dbp) return showToast('请输入血压值');
        if (Number(bp.sbp) <= Number(bp.dbp)) return showToast('收缩压应高于舒张压，请检查输入');
        body = { sbp: Number(bp.sbp), dbp: Number(bp.dbp), pulse: bp.pulse ? Number(bp.pulse) : undefined, period, measuredAt, tags };
      } else if (metric === 'lipid') {
        body = { ...Object.fromEntries(lipidItems.map(([k]) => [k, lipid[k] === '' ? undefined : Number(lipid[k])])), fasting: lipid.fasting, measuredAt };
      } else {
        if (!buf) return showToast('请输入尿酸值');
        body = { value: Number(buf), fasting, measuredAt };
      }
  ...
  return <div className={`sheet ${open ? 'on' : ''}`}><div className="sheet-h"><span className="t">记一笔</span><button className="x" onClick={close}>✕</button></div><div className="sheet-body"><div className="msg">{metrics.map((m) => <button className={metric === m.k ? 'on' : ''} key={m.k} onClick={() => setMetric(m.k)}>{m.n}</button>)}</div><div className="time-row"><input type="datetime-local" value={time} onChange={(e) => setTime(e.target.value)} /></div>{metric === 'bp' ? <BpPanel bp={bp} setBp={setBp} live={live} period={period} setPeriod={setPeriod} tags={tags} setTags={setTags} /> : metric === 'lipid' ? <LipidPanel lipid={lipid} setLipid={setLipid} live={live} /> : <BigPanel metric={metric} buf={buf} live={live} unit={metric === 'glucose' ? (me?.unit === 'mgdl' ? 'mg/dL' : 'mmol/L') : 'μmol/L'} period={period} setPeriod={setPeriod} tags={tags} setTags={setTags} fasting={fasting} setFasting={setFasting} />}</div>{metric !== 'lipid' ? <div className="keypad">{['1','2','3','save','4','5','6','7','8','9','.','0','del'].map((k) => k === 'save' ? <button key={k} className="key save" onClick={save}>保存记录</button> : <button key={k} className={`key ${k === '.' || k === 'del' ? 'fn' : ''}`} onClick={() => key(k)}>{k === 'del' ? '⌫' : metric === 'bp' && k === '.' ? '下一项' : k === '.' ? '·' : k}</button>)}</div> : <div id="lipidSaveBar" style={{ display: 'block', padding: '10px 16px 16px', background: '#EBF0ED', borderRadius: '22px 22px 0 0' }}><button className="btn primary" onClick={save}>保存记录</button></div>}</div>;
}
